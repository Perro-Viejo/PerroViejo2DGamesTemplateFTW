...
func set_action_list()->void:
	...
	for Action in list:
		...
		GUIManager.emit_signal("newScrollContainerButton", ActionNode) #emit to send node to ScrollContainer
...
func new_bind(Action, event)->void: #Adding bound InputEvent in the list
	...
	GUIManager.emit_signal("newScrollContainerButton", eventNode) #emit to send node to ScrollContainer
